...
class JSONEncodedObj(TypeDecorator):
    """ Represents an immutable structure as a json-encoded dictionary """
    impl = String
...
class MutableDict(MutableObject, dict):
    @classmethod
    def coerce(cls, key, value):
        """Convert plain dictionary to MutableDict"""
        self = MutableDict((k,MutableObject.coerce(key,v)) for (k,v) in value.items())
        self._key = key
        return self

    def __setitem__(self, key, value):
        dict.__setitem__(self, key, MutableObject.coerce(self._key, value))
        self.changed()
...
class MutableList(MutableObject, list):
    @classmethod
    def coerce(cls, key, value):
        """Convert plain list to MutableList"""
        self = MutableList((MutableObject.coerce(key, v) for v in value))
        self._key = key
        return self

    def __setitem__(self, idx, value):
        list.__setitem__(self, idx, MutableObject.coerce(self._key, value))
        self.changed()

    def __setslice__(self, start, stop, values):
        list.__setslice__(self, start, stop, (MutableObject.coerce(self._key, v) for v in values))
        self.changed()
    ...
    def append(self, value):
        list.append(self, MutableObject.coerce(self._key, value))
        self.changed()

    def insert(self, idx, value):
        list.insert(self, idx, MutableObject.coerce(self._key, value))
        self.changed()

    def extend(self, values):
        list.extend(self, (MutableObject.coerce(self._key, v) for v in values))
        self.changed()
...
def JSONAlchemy(sqltype):
    ...
    return MutableObject.as_mutable(_JSONEncodedObj)
